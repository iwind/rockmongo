			...
			<?php foreach ($arguments as $index => $argument): ?>
				<div>
					<?php hm("argument"); ?> #<span><?php h($index); ?></span>[JSON] <a href="#"
					                                                                    onclick="removeArgument(this);return false"><?php hm(
							"remove"
						); ?></a><br/><textarea name="argument[]" rows="5" cols="60"><?php h($argument); ?></textarea><br/>
				</div>
			<?php endforeach; ?>
		<?php endif; ?>
	</div>
	...
	<select name="db">
		<?php foreach ($dbs as $value): ?>
			<option value="<?php h($value["name"]); ?>"
			        <?php if (xn("db") == $value["name"]): ?>selected="selected"<?php endif; ?>><?php h(
					$value["name"]
				); ?></option>
		<?php endforeach; ?>

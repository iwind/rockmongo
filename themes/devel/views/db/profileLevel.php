...
<div>
	<form method="post">
		...
		<select name="level" onchange="changeLevel(this)">
			<option value="0" <?php if ($level == 0): ?>selected="selected"<?php endif; ?>><?php echo hm(
					"profiling_level1"
				); ?>:
			</option>
			<option value="1" <?php if ($level == 1): ?>selected="selected"<?php endif; ?>><?php echo hm(
					"profiling_level2"
				); ?></option>
			<option value="2" <?php if ($level == 2): ?>selected="selected"<?php endif; ?>><?php echo hm(
					"profiling_level3"
				); ?></option>
		</select><br/>

		<div id="slowmsDiv" style="display:none">
			<?php echo hm("timecost"); ?> &gt; <input type="text" name="slowms" size="7" value="<?php h(xi("slowms")); ?>"/>
			ms
		</div>

...
class DbController extends BaseController
{
	...
	public function doClearProfile()
	{
		...
		$this->redirect("db.profile", array("db" => $this->db));
	}
	...
	public function doDeleteUser()
	{
		...
		$this->redirect("db.auth", array("db" => $this->db));
	}
	...
	public function doAddUser()
	{
		...
		$db = $this->_mongo->selectDB($this->db);
		$db->execute("function (username, pass, readonly){ db.addUser(username, pass, readonly); }", array($username, $password, x("readonly") ? true : false));

		$this->redirect("auth", array("db" => $this->db));
	}

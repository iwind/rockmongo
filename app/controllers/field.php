...
class FieldController extends BaseController
{
	...
	function doNew()
	{
		...
		if (!$keep)
		{
			if ($id)
			{
				$db->selectCollection($this->collection)->update(array("_id" => rock_real_id($id)), array('$set' => array($newname => $realValue)));
			}
			else
			{
	...
	function doLoad()
	{
		...
		$this->_outputJson(array("code" => 200, "type" => $type, // long requires special handling (Kyryl Bilokurov <kyryl.bilokurov@gmail.com>)
			"value" => ($type == "long") ? $data->__toString() : $data, "represent" => $represent, "format" => $format));
	}
